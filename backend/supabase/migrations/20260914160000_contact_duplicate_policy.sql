-- Disabled during testing. Activation is an explicit database administration operation.
create table public.contact_request_policy (
  singleton boolean primary key default true check (singleton),
  block_duplicates boolean not null default false
);
insert into public.contact_request_policy(singleton, block_duplicates) values (true, false);
alter table public.contact_request_policy enable row level security;
revoke all on public.contact_request_policy from public, anon, authenticated, service_role;
create index contact_requests_phone_service_created_idx on public.contact_requests(phone, service_id, created_at desc);
