...
export function SiteHeader({ menuOpen, navItems, onCloseMenu, onHomeSection, onOpenMenu }) {
  return (
    <>
      <header className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-graphiteDark/78 text-white backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
          ...
          <a
            ...
            className="hidden items-center gap-2 rounded-md bg-pulse px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-pulse/20 transition hover:bg-orange-600 xl:inline-flex"
          >
            Reservar <CalendarCheck size={16} />
          </a>
          <button aria-label="Abrir menu" className="rounded-md border border-white/15 p-2 xl:hidden" onClick={onOpenMenu}>
            <Menu size={20} />
          </button>
        </div>
      </header>
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-[60] overflow-y-auto overscroll-contain bg-[#15181b] pb-8 text-white xl:hidden">
